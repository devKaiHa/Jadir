...
import useUpdateInvestmentFund from "../hooks/useUpdateInvestmentFund";
import LoadingCard from "../../../components/Global/LoadingCard";
import ErrorMessageCard from "../../../components/Global/ErrorMessageCard";

const UpdateInvestmentFund = () => {
  const {
    error,
    isPageLoading,
    isUpdating,
          ...
          <div className="grid gap-6 px-6 py-7 lg:grid-cols-[1.2fr_0.8fr] lg:px-8">
            <div>
              ...
              <h2 className="mt-4 text-2xl font-semibold">
                Refine the fund profile from the same structured editing workspace
              </h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-200">
                Keep fund metrics, linked companies, media, and multilingual copy
                aligned from one consistent layout.
              </p>
            </div>
        ...
        <div className="sticky bottom-4 z-20 mt-6 flex justify-end">
          <div className="rounded-2xl border border-gray-200 bg-white/95 p-3 shadow-lg backdrop-blur">
            <button
              ...
              disabled={isUpdating}
            >
              {isUpdating ? "Updating..." : "Update Investment Fund"}
            </button>
          </div>
        </div>
      </div>
...
export default UpdateInvestmentFund;
